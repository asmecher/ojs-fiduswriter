...
class FidusWriterSettingsForm extends Form {
	...
	function execute() {
		...
		$plugin->updateSetting($journalId, 'apiUrl', $this->getData('apiUrl'), 'string');
		$plugin->updateSetting($journalId, 'apiKey', $this->getData('apiKey'), 'string');
	}
	...
	function validate() {
		...
		$plugin =& $this->plugin;
		$plugin->import('FidusWriterConnection');
		$fidusWriterConnection = new FidusWriterConnection($this->getData('apiUrl'), $this->getData('apiKey'));
